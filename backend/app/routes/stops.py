...
def update_stop(day_id: int, stop_id: int, body: StopUpdate, session: Session = Depends(get_session), rider=Depends(require_admin)):
    ...
    if body.stop_type is not None: stop.stop_type = body.stop_type
...
def batch_create_stops(day_id: int, body: list, session: Session = Depends(get_session), rider=Depends(require_admin)):
    ...
    for i, stop_data in enumerate(body):
        name = _sanitize_text(stop_data.get("name", ""), max_length=200)
        stop = Stop(
            day_id=day_id, name=name, lat=float(stop_data.get("lat", 0)),
            lng=float(stop_data.get("lng", 0)), stop_type=stop_data.get("stop_type", "waypoint"),
            order_in_day=i,
        )
        session.add(stop)
        created.append(stop)
